...
import (
	...
	"pemuda-peduli/src/transaction/domain"
...
func (s *TransactionApp) Initialize(r *router.Router) {
	s.addRoute(r)
	log.Println("Transaction app initialized")
}

// Destroy will be called when app shutdowns
func (s *TransactionApp) Destroy() {
	// TODO Do clean up resource here
	log.Println("Transaction app released...")
}
...
func createTransaction(ctx *fasthttp.RequestCtx) {
	...
	if err := domain.CreateTransaction(ctx, DB, &data); err != nil {
...
func appliedTransaction(ctx *fasthttp.RequestCtx) {
	transactionID := fmt.Sprintf("%s", ctx.UserValue("id"))
	responseData, err := domain.AppliedPayment(ctx, DB, transactionID)
	if err != nil {
		ctx.SetStatusCode(fasthttp.StatusUnprocessableEntity)
		fmt.Fprintf(ctx, utility.PrettyPrint(handler.DefaultResponse(nil, err)))
		log.Println(err)
...
func declineTransaction(ctx *fasthttp.RequestCtx) {
	transactionID := fmt.Sprintf("%s", ctx.UserValue("id"))
	responseData, err := domain.DeclinePayment(ctx, DB, transactionID)
	if err != nil {
		ctx.SetStatusCode(fasthttp.StatusUnprocessableEntity)
		fmt.Fprintf(ctx, utility.PrettyPrint(handler.DefaultResponse(nil, err)))
		log.Println(err)
...
func uploadReceiptTransaction(ctx *fasthttp.RequestCtx) {
	payload, err := GetUploadReceiptPayload(ctx.Request.Body())
	if err != nil {
		ctx.SetStatusCode(fasthttp.StatusBadRequest)
		fmt.Fprintf(ctx, utility.PrettyPrint(handler.DefaultResponse(nil, errors.New("Bad JSON Payload"))))
		log.Println("Error Bad Request JSON Payload:", err)
	...
	responseData, err := domain.UploadPaymentReceipt(ctx, DB, payload.TransactionID, payload.ImageURL)
	if err != nil {
		ctx.SetStatusCode(fasthttp.StatusUnprocessableEntity)
		fmt.Fprintf(ctx, utility.PrettyPrint(handler.DefaultResponse(nil, err)))
		log.Println(err)
...
func cancelTransaction(ctx *fasthttp.RequestCtx) {
	transactionID := fmt.Sprintf("%s", ctx.UserValue("id"))
	responseData, err := domain.CancelPayment(ctx, DB, transactionID)
	if err != nil {
		ctx.SetStatusCode(fasthttp.StatusUnprocessableEntity)
		fmt.Fprintf(ctx, utility.PrettyPrint(handler.DefaultResponse(nil, err)))
		log.Println(err)
...
func findTransactions(ctx *fasthttp.RequestCtx) {
	...
	responseData, count, err := domain.FindTransaction(ctx, &repo, &data)
...
func findMyTransactions(ctx *fasthttp.RequestCtx) {
	...
	responseData, count, err := domain.FindMyTransaction(ctx, &repo, &data)
...
func getTransaction(ctx *fasthttp.RequestCtx) {
	transactionID := fmt.Sprintf("%s", ctx.UserValue("id"))
	repo := repository.NewTransactionRepository(DB)
	responseData, err := domain.GetTransaction(ctx, &repo, transactionID)
	if err != nil {
		ctx.SetStatusCode(fasthttp.StatusUnprocessableEntity)
		fmt.Fprintf(ctx, utility.PrettyPrint(handler.DefaultResponse(nil, err)))
		log.Println(err)
