...
type BeneficariesEntity struct {
	ID                 int64      `db:"id"`
	IDPPCPBeneficaries string     `db:"id_pp_cp_beneficaries"`
...
type BeneficariesQueryEntity struct {
	Limit         string `db:"limit"`
	Offset        string `db:"offset"`
	Filter        []BeneficariesFilterQueryEntity
...
type BeneficariesFilterQueryEntity struct {
	Field   string
	Keyword string
}
